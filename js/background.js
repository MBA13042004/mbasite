...
function init() {
    ...
    loader.load(
        ...
        function (texture) {
            cloudGeo = new THREE.PlaneGeometry(500, 500); // Changed PlaneBufferGeometry to PlaneGeometry for safety
...
function animate() {
    ...
    // Check if position attribute exists before accessing array
    if (rainGeo.attributes.position) {
        const positions = rainGeo.attributes.position.array;
        for (let i = 1; i < positions.length; i += 3) { // y is at index 1, 4, 7...
            positions[i] -= 0.1 + Math.random() * 0.1; // Fall down
            if (positions[i] < -200) {
